...
class VideoSummaryPDFGenerator:
            ...
            try:
                with open(temp_filename, "rb") as image_file:
                    ...
                    message = HumanMessage(content=[
                        {"type": "text", "text": "Describe what's happening in this video frame in 2-3 sentences."},
                ...
                summary = f"This segment shows content from {self._format_time(segment['start_time'])} to {self._format_time(segment['end_time'])}."
    ...
    def _generate_ai_commentary(self, summaries):
        ...
        try:
            message = HumanMessage(content=f"Based on these video segment summaries, provide insightful commentary about the overall content, themes, and significance of this video:\n\n{combined_summary}")
            
            # Get response from text model
            response = self.text_model.invoke([message])
            commentary = response.content
        except Exception as e:
            commentary = "AI commentary could not be generated due to an error."
    ...
    def _create_pdf(self, video_path, segments, summaries, ai_commentary):
        ...
        c.drawCentredString(width/2, height-50, f"Video Summary: {video_name}")
        
        # Add date
        c.setFont("Helvetica", 10)
        c.drawRightString(width-50, height-70, f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        
        # Add timeline segments
        c.setFont("Helvetica-Bold", 14)
        c.drawString(50, height-100, "Timeline Segments")
        
        # Extract thumbnail for each segment
        video = VideoFileClip(video_path)
        y_position = height - 130
        
        for _, (segment, summary) in enumerate(zip(segments, summaries)):
            ...
            c.drawString(50, y_position, f"{segment['name']} ({self._format_time(segment['start_time'])} - {self._format_time(segment['end_time'])})")
            ...
            c.drawImage(temp_filename, 50, y_position-100, width=100, height=75)
            
            # Add summary text next to thumbnail
            c.setFont("Helvetica", 10)
            textobject = c.beginText(160, y_position-20)
        ...
        c.showPage()
        c.setFont("Helvetica-Bold", 14)
        c.drawString(50, height-50, "AI Analysis")
        
        c.setFont("Helvetica", 10)
        textobject = c.beginText(50, height-80)
        textobject.textLines(ai_commentary)
        c.drawText(textobject)
